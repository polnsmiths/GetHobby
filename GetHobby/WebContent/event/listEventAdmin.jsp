...
/* 	$("#onGoing").on("click", function() {
	...
	$(document).on("click", ".page-link", function(){
		...
		if( $(this).attr("aria-label") == 'Previous' && !($(this).children().attr("class") == 'manage-no-page-span') ){
			...
			if( $("#").children("input[name='currentPage']").val() == 1){
        		$("#listEventAdmin").children("input[name='currentPage']").val(1);
        	}else{
...
<!-- 		            수정버튼
	            ...
	            <button type="button" class="btn btn-light button" style="border:0; outline:none; padding:0px; color:rgb(95, 174, 201);">새 이벤트 등록</button>
	            </div>
        ...
        if(chk) $(".select_subject input").prop('checked', true);
    			...
    			<tr>
					...
					<th></th>
      				...
      				<td>${i+1}</td>
      				<td id="eventTitle" class="event_title_class"><input type="hidden" id="eventId" name="eventId" value="${list[i].eventId }">${list[i].eventTitle }</input></td>
